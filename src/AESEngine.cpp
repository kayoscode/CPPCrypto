...
static inline void byteSubstitutionInv(unsigned char* state) {
    for(int i = 0; i < 16; i++) {
        state[i] = SBoxInv[(state[i] >> 4) & 0xF][state[i] & 0xF];
    }
}
...
static inline void mixColumnsInv(unsigned char* state) {
    ...
    unsigned char temp[16];
    for(int i = 0; i < 16; i += 4) {
        temp[i] = galMult(0xE, state[i]) ^ galMult(0xB, state[i + 1]) ^ galMult(0xD, state[i + 2]) ^ galMult(0x9, state[i + 3]);
        temp[i + 1] = galMult(0x9, state[i]) ^ galMult(0xE, state[i + 1]) ^ galMult(0xB, state[i + 2]) ^ galMult(0xD, state[i + 3]);
        temp[i + 2] = galMult(0xD, state[i]) ^ galMult(0x9, state[i + 1]) ^ galMult(0xE, state[i + 2]) ^ galMult(0xB, state[i + 3]);
        temp[i + 3] = galMult(0xB, state[i]) ^ galMult(0xD, state[i + 1]) ^ galMult(0x9, state[i + 2]) ^ galMult(0xE, state[i + 3]);
    }
...
void AESDecryptBlock128(unsigned char* state, unsigned char roundKeys[ROUNDS128][16]) {
    addRoundKey(state, roundKeys[ROUNDS128 - 1]);
    shiftRowsInv(state);
